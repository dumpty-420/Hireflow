...
class VectorStore:
        ...
        try:
            ...
            if PINECONE_INDEX_NAME in existing_indexes:
                ...
                if current_dimension == PINECONE_DIMENSION:
                    logger.info(
                        f"Index {PINECONE_INDEX_NAME} already exists with correct dimensions"
                    )
                    return True
                else:
                    logger.warning(
                        f"Index dimension mismatch: {current_dimension} vs {PINECONE_DIMENSION}"
                    )
                    self.client.delete_index(PINECONE_INDEX_NAME)
                    logger.info(
                        f"Deleted index {PINECONE_INDEX_NAME} due to dimension mismatch"
                    )
                    time.sleep(5)
            else:
        ...
        try:
            ...
            for i, doc in enumerate(documents):
                ...
                vectors.append(
                    {
                        ...
                        "values": self.embeddings.embed_documents([doc.page_content])[
                            0
                        ],
                        "metadata": metadata,
                    }
                ...
                if len(vectors) >= batch_size:
                    ...
                    logger.info(f"Upserted batch of {len(vectors)} resumes")
                    vectors = []
                    time.sleep(3)

            if vectors:
                self.index.upsert(vectors=vectors)
                logger.info(f"Upserted final batch of {len(vectors)} resumes")
    ...
    def add_job_descriptions(self, documents: List[Any]) -> bool:
        ...
        try:
            ...
            for i, doc in enumerate(documents):
                ...
                if len(vectors) >= batch_size:
                    self.index.upsert(vectors=vectors)
                    logger.info(f"Upserted batch of {len(vectors)} job descriptions")
                    vectors = []
                    time.sleep(1)

            if vectors:
                self.index.upsert(vectors=vectors)
                logger.info(f"Upserted final batch of {len(vectors)} job descriptions")
    ...
    def search_resumes(
        ...
        try:
            ...
            # Always filter by type=resume
            type_filter = {"type": {"$eq": "resume"}}
            if filters:
                pinecone_filters = self.convert_filters_to_pinecone(filters)
                pinecone_filters.update(type_filter)
            else:
                pinecone_filters = type_filter
    ...
    def search_job_descriptions(
        ...
        try:
            ...
            # Always filter by type=job_description
            type_filter = {"type": {"$eq": "job_description"}}
            if filters:
                pinecone_filters = self.convert_filters_to_pinecone(filters)
                pinecone_filters.update(type_filter)
            else:
        ...
        for key, value in filters.items():
            if isinstance(value, (str, int, float, bool)):
                ...
                if "min" in value and "max" in value:
                    pinecone_filters[key] = {"$gte": value["min"], "$lte": value["max"]}
                elif "min" in value:
