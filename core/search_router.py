...
class SearchRouter:
    ...
    def __init__(self, hybrid_indexer=None, vector_store=None):
                ...
                if self.vector_store.is_ready():
                    ...
                    for result in results:
                        formatted_results.append(
                            {
                                "candidate_id": result.get("metadata", {}).get(
                                    "candidate_id", "unknown"
                                ),
                                "name": result.get("metadata", {}).get(
                                    "name", "Unknown"
                                ),
        ...
        def deep_search(inputs: Dict[str, Any]) -> Dict[str, Any]:
            ...
            try:
                ...
                if results and self.llm:
                    # LLM reranking
                    reranked_results = self.llm_rerank(
                        results, query, jd_context, top_k
                    )
                    ...
                    formatted_results.append(
                        {
                            ...
                            "score": result.get(
                                "combined_rrf_score", result.get("rrf_score", 0.0)
                            ),
                            "metadata": result.get("metadata", {}),
                            "page_content": result.get(
                                "page_content",
                                result.get("metadata", {}).get("page_content", ""),
                            ),  # ✅ ADD THIS
    ...
    def llm_rerank(
        self, results: List[Dict], query: str, jd_context: str, top_k: int
    ) -> List[Dict]:
    ...
    def get_search_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "hybrid_indexer_ready": self.hybrid_indexer.get_index_stats()[
                "hybrid_ready"
            ],
