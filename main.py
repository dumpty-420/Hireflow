...
class HireFlowDemo:
    ...
    def check_prerequisites(self) -> bool:
        ...
        resume_count = len(
            [f for f in os.listdir(self.resume_dir) if f.endswith(".pdf")]
        )
    ...
    def test_search_modes(self):
        ...
        vector_results = self.components["vector_store"].search_resumes(
        ...
        print("\n2. Hybrid Search (BM25 + Vector)")
        hybrid_results = self.components["hybrid_indexer"].search_resumes(
            search_query, top_k=3
        )
        ...
        router_results = self.components["search_router"].search(
            search_query, top_k=3, search_mode="deep"
        )
    ...
    def test_ai_evaluation(self):
        ...
        results = self.components["search_router"].search(
            search_query, top_k=2, search_mode="deep"
        )
        ...
        try:
            parsed_jd = self.components["job_parser"].parse_job_description(
                sample_jd_text, "demo_jd"
            )
    ...
    def check_system_status(self):
        ...
        if self.components["hybrid_indexer"]:
            ...
            print(
                f"Resumes Indexed: {len(self.components['hybrid_indexer'].resume_texts)}"
            )
    ...
    def display_results(self, results: List[Dict], max_show: int = 3):
        ...
        for i, result in enumerate(results[:max_show], 1):
            ...
            print(f"   {i}. {name}")
            print(
                f"      Score: {score:.3f} | Experience: {experience} | Skills: {', '.join(skills[:3])}"
            )
